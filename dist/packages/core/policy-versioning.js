...
export function generateDiff(before, after, path = '') {
    const diff = {
        added: [],
        removed: [],
        modified: [],
...
export class PolicyVersioningManager {
    ...
    async saveVersion(record) {
        await this.pool.query(`INSERT INTO policy_versions 
    ...
    async getVersion(version) {
        const result = await this.pool.query(`SELECT * FROM policy_versions WHERE version = $1`, [version]);
    ...
    async getLatestVersion() {
        const result = await this.pool.query(`SELECT * FROM policy_versions 
     ...
     */
    async getVersionsInRetention(retentionDays = 365) {
        const result = await this.pool.query(`SELECT * FROM policy_versions 
     ...
     */
    parseVersionFromPolicy(policyContent) {
     ...
     */
    updateVersionInPolicy(policyContent, newVersion) {
        const policy = yaml.parse(policyContent);
    ...
    determineChangeType(diff) {
        ...
        const hasMajorChange = diff.modified.some((item) => item.path.includes('transforms') ||
        ...
        const hasMinorChange = diff.added.length > 0 ||
            diff.modified.some((item) => item.path.includes('threshold') ||
     ...
     */
    async createNewVersion(currentContent, newContent, userId, changeSummary) {
