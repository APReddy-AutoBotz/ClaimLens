...
describe('Transform Chain Integration Tests', () => {
    ...
    describe('Full Chain: detect → rewrite → redact', () => {
        it('should detect allergens, rewrite disclaimers, and redact PII in sequence', async () => {
            ...
            expect(verdict.changes.some(c => c.field === 'description' && c.after.includes('[EMAIL_REDACTED]'))).toBe(true);
        ...
        it('should handle banned claims with disclaimer and PII redaction', async () => {
            ...
            expect(verdict.reasons.some(r => r.transform === 'rewrite.disclaimer' && r.why.includes('banned claims'))).toBe(true);
            // Verify PII was redacted
            expect(verdict.reasons.some(r => r.transform === 'redact.pii' && r.why.includes('Redacted'))).toBe(true);
