 ...
 */
async function triggerWebhook(tenant, verdict, itemId) {
    try {
        // Get webhook configuration for tenant
        const result = await pool.query(`SELECT webhook_url, webhook_secret 
       FROM tenant_config 
       WHERE tenant = $1 AND webhook_url IS NOT NULL`, [tenant]);
...
router.post('/feed', (async (req, res) => {
    ...
    try {
        ...
        const result = await idempotencyManager.handleRequest(idempotencyKey, async () => {
            ...
            for (const item of itemsArray) {
                ...
                const verdict = await pipeline.execute(normalizedItem, 'menushield_in', {
