...
describe('SSRF Defense', () => {
  ...
  describe('safeFetch', () => {
    ...
    it('should enforce timeout', async () => {
      ...
      await expect(
        safeFetch('http://localhost:7001/slow')
    ...
    it('should check response size', async () => {
      ...
      global.fetch = vi.fn(() =>
        Promise.resolve({
          headers: {
            get: (name: string) => name === 'content-length' ? '10000000' : null,
          },
        } as any)
      );

      await expect(
        safeFetch('http://localhost:7001/large', {}, 1024)
