...
describe('WebhookManager', () => {
  ...
  describe('deliverWebhook - retry logic', () => {
    it('should mark as failed after 5 retry attempts', async () => {
      ...
      const payload = webhookManager.createPayload(
        'test-tenant',
        {
          verdict: 'allow',
          changes: [],
  ...
  describe('retryDelivery', () => {
    it('should throw error for non-existent record', async () => {
      ...
      await expect(
        webhookManager.retryDelivery('non-existent-id', config)
      ).rejects.toThrow('Webhook delivery record non-existent-id not found');
    });
  });
});
