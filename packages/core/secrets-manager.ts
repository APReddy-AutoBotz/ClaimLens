 ...
 */
const TENANT_KEYS = new Map<string, Buffer>();
 ...
 */
export function loadTenantKey(tenantId: string, keyBase64: string): void {
 ...
 */
export function encryptPII(tenantId: string, plaintext: string): string {
  if (!plaintext) {
    return '';
  }
 ...
 */
export function decryptPII(tenantId: string, encryptedBase64: string): string {
  if (!encryptedBase64) {
    return '';
  }
...
export async function rotateTenantKey(
  tenantId: string,
  reEncryptCallback: (oldDecrypt: (data: string) => string, newEncrypt: (data: string) => string) => Promise<void>
): Promise<string> {
  ...
  const oldDecrypt = (data: string) => decryptPII(tenantId, data);
  const newEncrypt = (data: string) => {
