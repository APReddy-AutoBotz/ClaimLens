...
export class DashboardMetricsCalculator {
   ...
   */
  async calculateKPIs(tenant: string): Promise<DashboardKPIs> {
    // Total audits processed
    const totalResult = await this.pool.query(
      `SELECT COUNT(*) as count FROM audits WHERE tenant_id = $1`,
      [tenant]
    );
    ...
    const flaggedResult = await this.pool.query(
      `SELECT COUNT(*) as count 
       FROM audits 
       WHERE tenant_id = $1 
       AND (verdict->>'verdict' = 'modify' OR verdict->>'verdict' = 'block')`,
      [tenant]
    );
    ...
    const avgTimeResult = await this.pool.query(
      `SELECT AVG(latency_ms) as avg_time 
       FROM audits 
       WHERE tenant_id = $1`,
      [tenant]
    );
    ...
    const last24HoursResult = await this.pool.query(
      `SELECT 
         COUNT(*) as audits,
    ...
    // Last 7 days metrics
    const last7DaysResult = await this.pool.query(
      `SELECT 
         COUNT(*) as audits,
         COUNT(CASE WHEN verdict->>'verdict' IN ('modify', 'block') THEN 1 END) as flagged,
         AVG(latency_ms) as avg_time
   ...
   */
  async getRecentAudits(tenant: string, limit: number = 20): Promise<RecentAudit[]> {
    ...
    return result.rows.map((row) => ({
      ...
      verdict: row.verdict as 'allow' | 'modify' | 'block',
   ...
   */
  async getDegradedServices(tenant: string): Promise<string[]> {
    const result = await this.pool.query(
      `SELECT DISTINCT degraded_services 
       FROM audits 
       WHERE tenant_id = $1 
  ...
  async getPerformanceTrends(
    tenant: string,
    days: number = 7
  ): Promise<Array<{ date: string; avgLatencyMs: number; count: number }>> {
    const result = await this.pool.query(
      `SELECT 
         DATE(ts) as date,
