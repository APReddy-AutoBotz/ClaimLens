...
import { User, UserSession, Tenant, TenantConfig, ApiKey } from './tenant-models';
...
export class UserRepository {
  ...
  async createTenantConfig(config: Partial<TenantConfig>): Promise<TenantConfig> {
    const result = await this.pool.query(
      `INSERT INTO tenant_config (tenant, retention_days, webhook_url, webhook_secret, allowed_locales, custom_rule_packs)
  ...
  async getTenantConfig(tenant: string): Promise<TenantConfig | null> {
    const result = await this.pool.query(
      `SELECT * FROM tenant_config WHERE tenant = $1`,
      [tenant]
    );
    return result.rows[0] || null;
  }
  ...
  async createUserSession(userId: string, token: string): Promise<UserSession> {
  ...
  async createApiKey(
    ...
    const result = await this.pool.query(
      `INSERT INTO api_keys (tenant, key_hash, name, expires_at)
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [tenant, keyHash, name, expiresAt]
    );
    return result.rows[0];
  }
