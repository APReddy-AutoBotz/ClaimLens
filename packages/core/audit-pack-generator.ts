...
import type { AuditRecord, Change, Reason } from './types.js';
...
export interface AuditPackSummary {
  ...
  flags_by_transform: Record<string, number>;
...
export class AuditPackGenerator {
  private options: AuditPackOptions;

  constructor(options: AuditPackOptions = {}) {
    this.options = {
  ...
  async generate(
    records: AuditRecord[],
    packId?: string
  ): Promise<{ jsonlPath: string; mdPath: string; summary: AuditPackSummary }> {
    if (records.length === 0) {
   ...
   */
  private async generateJSONL(records: AuditRecord[], outputPath: string): Promise<void> {
   ...
   */
  private calculateSummary(records: AuditRecord[]): AuditPackSummary {
    const latencies = records.map(r => r.latency_ms).sort((a, b) => a - b);
    const flagsByTransform: Record<string, number> = {};
    let totalChanges = 0;
