...
import type { TransformPipeline } from './pipeline.js';
import type { NormalizedMenuItem, Verdict } from './types.js';
...
export class FixturesRunner {
  constructor(
    ...
    private fixturesPath: string = 'fixtures'
  ) {}

  /**
   * List available fixtures
   */
  async listFixtures(): Promise<{ menu: string[]; sites: string[] }> {
  ...
  async runFixtures(
    ...
    for (const fixture of menuFixtures) {
      ...
      for (const item of data.items) {
        ...
        try {
          const verdict = await this.pipeline.execute(
            item as NormalizedMenuItem,
            profile,
            {
              locale: 'en-IN',
              tenant: 'test',
    ...
    const summary: FixtureRunSummary = {
   ...
   */
  async validateFixture(fixturePath: string): Promise<{
    valid: boolean;
    errors: string[];
  }> {
    const errors: string[] = [];
    ...
    try {
      ...
      errors.push(
        `Failed to parse fixture: ${error instanceof Error ? error.message : String(error)}`
      );
    }
