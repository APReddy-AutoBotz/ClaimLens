...
let bannedClaimsCache: string[] | null = null;

/**
 * Load banned claims from packs/banned.claims.in.yaml
 */
function loadBannedClaims(): string[] {
  ...
  try {
    ...
    bannedClaimsCache = parse(content) as string[];
    return bannedClaimsCache;
  } catch (error) {
...
export type ClaimCategory = 'health' | 'nutrition' | 'marketing';
 ...
 */
function classifyClaim(claim: string): ClaimCategory {
 ...
 */
function getDisclaimer(category: ClaimCategory, locale: string): string {
  const disclaimers: Record<string, Record<ClaimCategory, string>> = {
    'en-IN': {
 ...
 */
export function rewriteDisclaimer(text: string, locale: string = 'en-IN'): DisclaimerResult {
  ...
  const foundClaims = bannedClaims.filter(claim => 
 ...
 */
export function rewriteDisclaimerTransform(
  input: string,
