...
import type { AuditRecord, Verdict, NormalizedMenuItem } from '../../../packages/core/types';
 ...
 */
async function createTestAuditRecords(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const snapshot = auditManager.createSnapshot(
      item,
      `Description ${i}`,
      i % 2 === 0 ? `Modified Description ${i}` : `Description ${i}`
    );

    const record = auditManager.createAuditRecord(
      auditId,
      {
        tenant,
