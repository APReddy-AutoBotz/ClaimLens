...
import type { AuditStorageBackend } from '../../../packages/core/audit-manager';
...
let auditStorage: AuditStorageBackend = new InMemoryAuditStorage();

/**
 * Set the audit storage backend (for testing)
 */
export function setAuditStorage(storage: AuditStorageBackend): void {
...
function checkExportRateLimit(key: string): boolean {
  const now = Date.now();
 ...
 */
router.get('/menu.ndjson', (async (req: AuthenticatedRequest, res: Response) => {
  try {
    // Apply rate limiting
