...
import { AugmentLiteGate, AugmentLiteFields } from '../../../packages/core/augment-lite-gate';
...
import { AuditManager, AuditStorageBackend } from '../../../packages/core/audit-manager';
...
const auditStorage: AuditStorageBackend = new InMemoryAuditStorage();
...
const fixturesRunner = new FixturesRunner(transformPipeline);
  ...
  router.get(
    '/audit-trail',
    requirePermission('audits', 'read') as any,
    (async (req: AuthenticatedRequest, res: Response) => {
      try {
        const limit = parseInt(req.query.limit as string) || 100;
        const offset = parseInt(req.query.offset as string) || 0;

        const changes = await changeLogger.getChangeHistory(
          req.tenant!,
          limit,
          offset
        );
  ...
  router.get(
    ...
    (async (req: AuthenticatedRequest, res: Response) => {
      try {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="audit-trail-${req.tenant}-${Date.now()}.csv"`
        );
        res.send(csv);
      } catch (error) {
        res.status(500).json({
  ...
  router.get(
    '/profiles',
    requirePermission('policies', 'read') as any,
    (async (req: AuthenticatedRequest, res: Response) => {
      try {
  ...
  router.put(
    '/profiles/:id',
    requirePermission('policies', 'write') as any,
    (async (req: AuthenticatedRequest, res: Response) => {
      try {
        ...
        const validation = augmentLiteGate.validateEdit(
          action,
          augmentLite as AugmentLiteFields,
          autonomy
        );
        ...
        // Create new version
        const versionRecord = await versioningManager.createNewVersion(
          currentContent,
          newContent,
          req.user!.id,
          augmentLite.context
        ...
        await changeLogger.logChange(req.user!, {
          ...
          augmentLite: augmentLite as AugmentLiteFields,
        });
  ...
  router.get(
    '/rule-packs',
    requirePermission('rule_packs', 'read') as any,
    (async (req: AuthenticatedRequest, res: Response) => {
      try {
        const packs = [
  ...
  router.put(
    '/rule-packs/:name',
    requirePermission('rule_packs', 'write') as any,
    (async (req: AuthenticatedRequest, res: Response) => {
      try {
        ...
        // Validate Augment-Lite fields
        const validation = augmentLiteGate.validateFields(augmentLite as AugmentLiteFields);
