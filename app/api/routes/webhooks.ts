...
import { TenantConfig } from '../../../packages/core/tenant-models';
...
router.post(
  '/config',
  requirePermission('webhooks', 'write') as any,
    ...
    try {
      ...
      await pool.query(
        `UPDATE tenant_config 
         ...
         WHERE tenant = $3`,
        [webhook_url, webhook_secret, req.tenant]
      );
...
router.get(
  '/config',
  requirePermission('webhooks', 'read') as any,
  (async (req: AuthenticatedRequest, res: Response) => {
    try {
