...
export class BadgeRenderer {
   ...
   */
  applyBadges(badges: Badge[]): void {
    badges.forEach(badge => {
      try {
        const element = document.querySelector(badge.dom_selector);
        if (element) {
          this.renderBadge(element as HTMLElement, badge);
        }
      } catch (error) {
   ...
   */
  private renderBadge(element: HTMLElement, badge: Badge): void {
   ...
   */
  private applyBadgeStyles(element: HTMLElement, kind: Badge['kind']): void {
    const baseStyles = `
      display: inline-flex;
      align-items: center;
      padding: 4px 8px;
    ...
    const kindStyles: Record<Badge['kind'], string> = {
   ...
   */
  private positionBadge(parent: HTMLElement, badge: HTMLElement): void {
   ...
   */
  private setupTooltip(badgeEl: HTMLElement, badge: Badge): void {
    const showTooltip = () => {
    ...
    document.addEventListener('click', (e) => {
      if (!this.tooltips.has(badge.item_id)) return;
      
      const currentTooltip = this.tooltips.get(badge.item_id);
      if (currentTooltip && !currentTooltip.contains(e.target as Node) && !badgeEl.contains(e.target as Node)) {
        hideTooltip();
      }
    });
  }
   ...
   */
  private createTooltip(badge: Badge): HTMLElement {
  ...
  private applyTooltipStyles(tooltip: HTMLElement): void {
    ...
    // Style explanation
    const explanation = tooltip.querySelector('.claimlens-tooltip-explanation') as HTMLElement;
    if (explanation) {
    ...
    // Style source link
    const source = tooltip.querySelector('.claimlens-tooltip-source') as HTMLElement;
    if (source) {
    ...
    // Style close button
    const closeBtn = tooltip.querySelector('.claimlens-tooltip-close') as HTMLElement;
    if (closeBtn) {
   ...
   */
  private positionTooltip(badgeEl: HTMLElement, tooltip: HTMLElement): void {
   ...
   * Hide tooltip
   */
  private hideTooltip(itemId: string): void {
