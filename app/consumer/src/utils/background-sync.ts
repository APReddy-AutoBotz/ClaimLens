 ...
 * Add scan to queue
 */
export function queueScan(scan: Omit<QueuedScan, 'id' | 'timestamp' | 'status' | 'retryCount'>): string {
 ...
 */
function storeSyncedResult(scanId: string, result: any): void {
  try {
...
export async function registerBackgroundSync(): Promise<void> {
  ...
  try {
    ...
    if ('sync' in registration) {
      await (registration as any).sync.register('sync-scans');
